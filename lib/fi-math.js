...
window.FIMath = (function () {
  ...
  // Overrides via inputs.socialSecurity: { claimAge, monthly }.
  function socialSecurity(inputs, opts = {}) {
    ...
    if (ss.monthly > 0) {
      ...
      const aime = (Math.min(annualIncome ?? 0, SS_WAGE_CAP) * (yearsWorked / 35)) / 12;
      const pia =
    ...
    return { enabled: inputs.includeSS === true, birthYear, fra, claimAge, monthly, annual: monthly * 12 };
  }
