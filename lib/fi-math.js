...
window.FIMath = (function () {
  ...
  // With inputs.couple, each partner has their own age, income share, and
  // retirement year, giving three household phases:
  //   1. both working    — contributions = annualSavings (savingsRate applies)
  //   2. one working     — flow = worker's income − expenses (portfolio absorbs
  //                        the gap; employer coverage assumed, no HC bridge)
  //   3. both retired    — withdrawals, HC bridge per person under Medicare age,
  //                        each partner's SS starting at their own claim age
  function monteCarlo(inputs, opts = {}) {
    const {
      ...
      retirementYear, partnerRetirementYear,
    } = inputs;
    ...
    // Couple: independent partner age, income share, and retirement year
    const { couple, myIncome, partnerIncome, partnerAge } = householdSplit(inputs);
    const partnerRetYear = couple ? (partnerRetirementYear ?? retYear) : retYear;
    const partnerAgeAtYear = (y) => partnerAge != null ? partnerAge + y : ageAtYear(y);
    ...
    // Social Security — market-independent, COLA-indexed income per person,
    // each stream starting at that person's own claim age. Never subject to
    // the random return draw; it only shrinks (or reverses) the withdrawal
    // that the portfolio must fund.
    const ssInfo = socialSecurity(inputs, {
      retireAge: currentAge != null ? currentAge + retYear : undefined,
      partnerRetireAge: (couple && partnerAge != null) ? partnerAge + partnerRetYear : undefined,
      currentYear: opts.currentYear,
    });
    const ssStreams = ssInfo.enabled
      ? ssInfo.people.map((p, i) => ({
          annual: p.monthly * 12,
          startAge: p.claimAge,
          ageAt: i === 0 ? ageAtYear : partnerAgeAtYear,
        })).filter(st => st.annual > 0)
      : [];
    ...
    for (let t = 0; t < trials; t++) {
      ...
      for (let y = 1; y <= yearsToProject; y++) {
        ...
        const ssThisYear = ssStreams.reduce(
          (sum, st) => sum + (st.ageAt(y) >= st.startAge ? st.annual : 0), 0);
        const worksA = y <= retYear;
        const worksB = couple && y <= partnerRetYear;
        if (worksA && (!couple || worksB)) {
          // Everyone still working. Negative balances are allowed
          // pre-retirement (net debt being paid down).
          const preRetPI = (pi.preRetirement !== false) ? piThisYear : 0;
          bal = bal * (1 + r) + annualSavings + preRetPI + ssThisYear - liabOutflow;
        } else if (worksA || worksB) {
          // One partner retired: the worker's income covers household spending
          // and the portfolio absorbs the surplus or shortfall. savingsRate
          // only governs the both-working phase.
          const workerIncome = worksA ? myIncome : partnerIncome;
          const preRetPI = (pi.preRetirement !== false) ? piThisYear : 0;
          bal = bal * (1 + r) + workerIncome - annualExpenses + preRetPI + ssThisYear - liabOutflow;
        } else {
          // Fully retired household. HC bridge is per person still under
          // Medicare age (employer coverage assumed while either works).
          const underMedicare = (ageAtYear(y) <= hcMedicareAge ? 1 : 0)
            + (couple ? (partnerAgeAtYear(y) <= hcMedicareAge ? 1 : 0) : 0);
          const hcExtra = bridgeAnnual * underMedicare / (couple ? 2 : 1);
  ...
  // Per-person household earnings split. When couple is on, annualIncome stays
  // the household total; partner.income is the partner's share (null = 50/50).
  function householdSplit(inputs) {
    const couple = inputs.couple === true;
    const partner = inputs.partner || {};
    const total = inputs.annualIncome ?? 0;
    const partnerIncome = couple
      ? Math.min(Math.max(partner.income ?? total / 2, 0), total)
      : 0;
    return {
      couple,
      myIncome: total - partnerIncome,
      partnerIncome,
      partnerAge: partner.age ?? inputs.currentAge,
    };
  }

  // Estimated Social Security benefit, derived from existing inputs:
  // - claim age defaults to each person's full retirement age (from birth year)
  ...
  // - inputs.couple computes an independent benefit per partner from their own
  //   age, income share, and retirement age (the bend points are progressive
  //   and the wage cap is per person)
  // Overrides via inputs.socialSecurity: { claimAge, monthly } — monthly is
  // the household total at the claim age.
  // Returns { monthly, annual, claimAge, fra, people: [{ age, fra, claimAge, monthly }] }.
  function socialSecurity(inputs, opts = {}) {
    ...
    const benefitFor = (age, income, retireAge) => {
      const birthYear = nowYear - (age ?? 40);
      const fra = ssFullRetirementAge(birthYear);
      const claimAge = Math.min(70, Math.max(62, ss.claimAge ?? fra));
      const careerStart = 22;
      const yearsWorked = Math.max(0, Math.min(35, Math.max(retireAge ?? fra, age ?? 0) - careerStart));
      const aime = (Math.min(income ?? 0, SS_WAGE_CAP) * (yearsWorked / 35)) / 12;
      const pia =
      ...
      const factor = months < 0
        ...
        : 1 + months * (8 / 1200);
      return { age, birthYear, fra, claimAge, monthly: pia * factor };
    };

    const myRetireAge = opts.retireAge ?? inputs.targetAge;
    // Partner defaults to retiring in the same calendar year
    const partnerRetireAge = opts.partnerRetireAge ??
      (myRetireAge != null && currentAge != null ? partnerAge + (myRetireAge - currentAge) : myRetireAge);

    const people = couple
      ? [benefitFor(currentAge, myIncome, myRetireAge), benefitFor(partnerAge, partnerIncome, partnerRetireAge)]
      : [benefitFor(currentAge, inputs.annualIncome, myRetireAge)];

    if (ss.monthly > 0) {
      // explicit household override, split evenly across people for start timing
      const each = ss.monthly / people.length;
      people.forEach(p => { p.monthly = each; });
    }
    const monthly = people.reduce((sum, p) => sum + p.monthly, 0);
    return {
      enabled: inputs.includeSS === true,
      birthYear: people[0].birthYear, fra: people[0].fra, claimAge: people[0].claimAge,
      monthly, annual: monthly * 12, couple, people,
    };
  }
  ...
  return {
    ...
    socialSecurity, ssFullRetirementAge, householdSplit,
